...
trait WithValidate
{
    ...
    protected static function checkRule(string $field, mixed $value, string $rule): ?string
    {
        ...
        switch ($ruleName) {
            ...
            case "boolean":
                if ($value !== null && $value !== '' && !is_bool($value) && !in_array($value, [0, 1, '0', '1'], true)) {
                    return $customMessage ?? "Il campo {$field} deve essere booleano";
                }
                break;
            ...
            case 'in':
                if ($value !== null) {
                    ...
                    if (!in_array((string)$value, $allowed, true) && !in_array((int)$value, array_map('intval', $allowed), true)) {
                        return $customMessage ?? "Il campo {$field} deve essere uno dei seguenti valori: " . implode(', ', $allowed);
                    }
                }
                break;
            ...
            case 'string':
                if ($value !== null && !is_string($value)) {
                    return $customMessage ?? "Il campo {$field} deve essere una stringa";
                }
                break;

            case 'integer':
                if ($value !== null && filter_var($value, FILTER_VALIDATE_INT) === false) {
                    return $customMessage ?? "Il campo {$field} deve essere un numero intero";
                }
                break;

            case 'numeric':
                if ($value !== null && !is_numeric($value)) {
                    return $customMessage ?? "Il campo {$field} deve essere numerico";
                }
                break;
            ...
            case 'regex':
                if ($value !== null && !preg_match($param, $value)) {
                    return $customMessage ?? "Il campo {$field} ha un formato non valido";
                }
                break;
        }
